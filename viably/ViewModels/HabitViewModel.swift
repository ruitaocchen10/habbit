...
class HabitViewModel {
    ...
    // MARK: - MVD Computed Properties

    var mvdTemplates: [HabitTemplate] {
        activeTemplates.filter { $0.isMvd }
    }

    var mvdProgress: (completed: Int, total: Int) {
        let total = mvdTemplates.count
        let completed = mvdTemplates.filter { completionsForDate.contains($0.id) }.count
        return (completed, total)
    }

    var isMvdComplete: Bool {
        let total = mvdTemplates.count
        guard total > 0 else { return false }
        return mvdProgress.completed == total
    }
